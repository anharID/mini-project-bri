...
@section('container')
<h1 class="h3 mb-2 text-gray-800">Ubah Data Materi</h1>
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <form class="" action="{{ route('data-materi.update', $data_materi->id) }}" method="POST">
            @method('put')
            @csrf
            <div class="">
                ...
                <input type="text" name="materi" class="form-control @error('materi') is-invalid @enderror" id="materi"
                    placeholder="Nama Kelas" required autofocus value="{{ old('materi', $data_materi->materi) }}">
                @error('materi')
